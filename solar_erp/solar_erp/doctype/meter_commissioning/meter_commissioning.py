# Copyright (c) 2025, Your Company and contributors
# For license information, please see license.txt

import frappe
from frappe import _
from frappe.model.document import Document


class MeterCommissioning(Document):
    def validate(self):
        """Validate the document before saving"""
        # Prevent manual creation without meter_installation link
        if self.is_new() and not self.meter_installation:
            # Allow only if created by system (ignore_permissions flag is set)
            if not self.flags.ignore_permissions:
                frappe.throw(
                    _("Meter Commissioning can only be created automatically when Meter Installation is approved. "
                      "Please approve the Meter Installation record first."),
                    title=_("Manual Creation Not Allowed")
                )

    def before_save(self):
        """Store previous status for comparison"""
        if not self.is_new():
            self._previous_status = frappe.db.get_value("Meter Commissioning", self.name, "status")
        else:
            self._previous_status = None
    
    def on_update(self):
        """Trigger auto-creation of Verification Handover when approved"""
        previous_status = getattr(self, '_previous_status', None)
        
        # Check if status changed to Approved
        if self.status == "Approved" and previous_status != "Approved":
            self.handle_approval()
    
    def handle_approval(self):
        """Handle Meter Commissioning approval - create Verification Handover"""
        # Create Verification Handover if it doesn't exist
        verification_handover = self.create_verification_handover_if_not_exists()
        
        # Link the Verification Handover
        if verification_handover:
            frappe.db.set_value("Meter Commissioning", self.name, "linked_verification_handover", verification_handover.name, update_modified=False)
        
        # Commit the transaction
        frappe.db.commit()
        
        # Log audit entry
        self.log_approval_audit(verification_handover)
    
    def create_verification_handover_if_not_exists(self):
        """Create Verification Handover record if it doesn't exist for this Job File"""
        # Check if Verification Handover already exists for this Job File
        existing_vh = frappe.db.exists("Verification Handover", {"job_file": self.job_file})
        
        if existing_vh:
            frappe.msgprint(
                _("Verification Handover {0} already exists for Job File {1}").format(existing_vh, self.job_file),
                alert=True
            )
            return frappe.get_doc("Verification Handover", existing_vh)
        
        # Create new Verification Handover
        verification_handover = frappe.get_doc({
            "doctype": "Verification Handover",
            "job_file": self.job_file,
            "lead": self.lead,
            "project": self.project,
            "meter_commissioning": self.name,
            "status": "Draft"
        })
        
        verification_handover.flags.ignore_permissions = True
        verification_handover.insert()
        
        frappe.msgprint(
            _("Verification Handover {0} created successfully").format(verification_handover.name),
            alert=True,
            indicator="green"
        )
        
        return verification_handover
    
    def log_approval_audit(self, verification_handover=None):
        """Log audit entry for Meter Commissioning approval"""
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "Meter Commissioning",
            "reference_name": self.name,
            "content": _("Meter Commissioning approved.{0}").format(
                _(" Verification Handover {0} created.").format(verification_handover.name) if verification_handover else ""
            )
        }).insert(ignore_permissions=True)


@frappe.whitelist()
def start_work(docname):
    """Start work - transitions from Draft/Rework to In Progress"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status not in ["Draft", "Rework"]:
        frappe.throw(_("Can only start work when status is Draft or Rework"))
    
    # Check if user has the right role
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Executive"}) or
            frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Manager"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("You don't have permission to start this work"))
    
    doc.status = "In Progress"
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def submit_work(docname):
    """Submit work for review - transitions from In Progress to Ready for Review"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status != "In Progress":
        frappe.throw(_("Can only submit work when status is In Progress"))
    
    # Check if user has the right role (Executive only)
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Executive"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Executive can submit work for review"))
    
    doc.status = "Ready for Review"
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def hold_work(docname, reason):
    """Put work on hold - transitions to Blocked status (Executive action)"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status != "In Progress":
        frappe.throw(_("Can only hold when status is In Progress"))
    
    # Check if user has the right role
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Executive"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Executive can put work on hold"))
    
    if not reason or reason.strip() == "":
        frappe.throw(_("Please provide a reason for putting on hold"))
    
    doc.status = "Blocked"
    doc.workflow_reason = reason
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def approve_work(docname, reason):
    """Approve work - transitions from Ready for Review to Approved"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status != "Ready for Review":
        frappe.throw(_("Can only approve when status is Ready for Review"))
    
    # Check if user has the right role (Manager only)
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Manager"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Manager can approve work"))
    
    if not reason or reason.strip() == "":
        frappe.throw(_("Please provide a reason for approval"))
    
    doc.status = "Approved"
    doc.workflow_reason = reason
    doc.reviewer = frappe.session.user
    doc.review_decision = "Approved"
    doc.review_comments = reason
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def block_work(docname, reason):
    """Block work - transitions to Blocked status (Manager action)"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status not in ["In Progress", "Ready for Review"]:
        frappe.throw(_("Can only block when status is In Progress or Ready for Review"))
    
    # Check if user has the right role (Manager only)
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Manager"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Manager can block work"))
    
    if not reason or reason.strip() == "":
        frappe.throw(_("Please provide a reason for blocking"))
    
    doc.status = "Blocked"
    doc.workflow_reason = reason
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def request_rework(docname, reason):
    """Request rework - transitions from Ready for Review to Rework"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status != "Ready for Review":
        frappe.throw(_("Can only request rework when status is Ready for Review"))
    
    # Check if user has the right role (Manager only)
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Manager"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Manager can request rework"))
    
    if not reason or reason.strip() == "":
        frappe.throw(_("Please provide a reason for rework"))
    
    doc.status = "Rework"
    doc.workflow_reason = reason
    doc.reviewer = frappe.session.user
    doc.review_decision = "Rework"
    doc.review_comments = reason
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def resume_work(docname, reason):
    """Resume work - transitions from Blocked to In Progress"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status != "Blocked":
        frappe.throw(_("Can only resume when status is Blocked"))
    
    # Check if user has the right role (Manager only)
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Manager"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Manager can resume work"))
    
    if not reason or reason.strip() == "":
        frappe.throw(_("Please provide a reason for resuming"))
    
    doc.status = "In Progress"
    doc.workflow_reason = reason
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}


@frappe.whitelist()
def close_work(docname, reason):
    """Close work - transitions from Approved to Closed"""
    doc = frappe.get_doc("Meter Commissioning", docname)
    
    if doc.status != "Approved":
        frappe.throw(_("Can only close when status is Approved"))
    
    # Check if user has the right role (Manager only)
    if not (frappe.db.exists("Has Role", {"parent": frappe.session.user, "role": "Meter Commissioning Manager"}) or
            "System Manager" in frappe.get_roles() or
            "Administrator" in frappe.get_roles()):
        frappe.throw(_("Only Meter Commissioning Manager can close work"))
    
    if not reason or reason.strip() == "":
        frappe.throw(_("Please provide a reason for closing"))
    
    doc.status = "Closed"
    doc.workflow_reason = reason
    doc.save()
    frappe.db.commit()
    
    return {"status": "success"}
